...
export async function registerRoutes(
  ...
  app.post("/api/orders", requireAuth, async (req, res) => {
    try {
      ...
      // plate cost AT SALE TIME (shared/menuCost.ts via storage.computeMenuItemCost) —
      // null when no full recipe cost is known, never a fabricated 0.
      const itemsToInsert = await Promise.all(lineItems.map(async (item: any, idx: number) => {
        const costResult = await storage.computeMenuItemCost(Number(item.menuItemId), item.size || null);
        return {

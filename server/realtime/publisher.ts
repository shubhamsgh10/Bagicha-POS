...
export class PusherPublisher implements RealtimePublisher {
  private pusher: { trigger: (channel: string, event: string, data: unknown) => Promise<unknown> };
  private channel: string;

  constructor(pusher: PusherPublisher["pusher"], channel: string) {
    this.pusher = pusher;
...
export async function createRealtimePublisher(
  ...
  const channel = process.env.PUSHER_CHANNEL || "bagicha-pos";

...
function OrderDetailRow({ order, onStatusChange }: { order: any; onStatusChange: (id: number, status: string) => void }) {
  ...
  return (
    <motion.div
      ...
      className="rounded-2xl backdrop-blur-lg bg-white/40 border border-white/30 shadow-md overflow-hidden
                 hover:scale-[1.005] hover:shadow-xl hover:shadow-emerald-500/10
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {order.tableNumber && (
                <span className="text-[11px] bg-emerald-100/80 text-emerald-700 px-2 py-0.5 rounded-lg font-semibold backdrop-blur-sm">
        ...
        <div className="flex items-center gap-2 shrink-0 ml-3">
          ...
          <div onClick={(e) => e.stopPropagation()}>
            <Select value={order.status} onValueChange={(v) => onStatusChange(order.id, v)}>
              <SelectTrigger className="h-7 text-xs w-28 rounded-xl bg-white/50 border-white/40 backdrop-blur-sm">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
            ...
            <div className="border-t border-white/30 bg-white/20 backdrop-blur-sm px-4 py-3 space-y-3">
              ...
              <div>
                ...
                {items.length === 0 ? (
                  ...
                  <div className="rounded-xl overflow-hidden border border-white/30 bg-white/40 backdrop-blur-sm">
              ...
              <div className="flex justify-end">
                <div className="text-xs space-y-1 w-48 bg-white/30 rounded-xl px-3 py-2 backdrop-blur-sm">
    ...
    return (
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-6 space-y-3">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="h-16 rounded-2xl bg-white/30 border border-white/20 backdrop-blur-sm animate-pulse" />
          ))}
        </div>
      </div>
    );
    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-gradient-to-br from-slate-50 via-white to-emerald-50/30">
      ...
      <div className="px-6 pt-4 flex items-center gap-2 flex-wrap">
        ...
        <motion.button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-sm font-medium
                     bg-white/60 backdrop-blur-sm border border-white/40 text-gray-600
        ...
        {/* Search */}
        <div className="flex items-center gap-2 bg-white/60 backdrop-blur-sm border border-white/40
      ...
      <main className="min-h-0 flex-1 overflow-y-auto custom-scrollbar px-6 pt-4 pb-6">
        <Tabs defaultValue="all" className="w-full">
          <TabsList className="grid w-full grid-cols-6 rounded-xl bg-white/40 backdrop-blur-sm border border-white/30 p-1 mb-4">

...
const STEPS: {
  ...
  { key: "dispatched", label: "Dispatched", emoji: "🛵",
    ...
    doneCls:   "bg-blue-50   text-blue-600  border-blue-200"  },
  { key: "delivered",  label: "Delivered",  emoji: "✓",
    activeCls: "bg-emerald-600 text-white border-emerald-600",
    doneCls:   "bg-emerald-50  text-emerald-700 border-emerald-200" },
];
...
export const DeliveryCard = memo(function DeliveryCard({
  ...
  const statusIdx = DELIVERY_STATUS_ORDER.indexOf(status);
    ...
    >
      ...
      <div className="flex items-center justify-between gap-1">
        <div className="flex items-center gap-1.5 min-w-0">
          <span className="text-[10px] font-bold bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded-full truncate max-w-[120px]">
            #{order.orderNumber}
          </span>
          {order.hasNewItems && (
          ...
          {expanded && (
            ...
            >
              <div className="mt-2 space-y-1.5">
                ...
                {order.notes && (
                  <p className="text-[9px] text-gray-400 italic pt-1 border-t border-black/5">
                    Note: {order.notes}
                  </p>
                )}
              </div>
